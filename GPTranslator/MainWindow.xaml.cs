...
namespace GPTranslator
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void BaşlangıçtaÇalıştır(bool isChecked)
        {
            try
            {
                using (var registryKey = Registry.CurrentUser.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", true))
                {
                    if (isChecked)
                    {
                        registryKey.SetValue("GPTranslator", $@"""{Process.GetCurrentProcess().MainModule.FileName}""");
                    }
                    else
                    {
                        registryKey.DeleteValue("GPTranslator");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void MenuItem_Checked(object sender, RoutedEventArgs e) => BaşlangıçtaÇalıştır(true);

        private void MenuItem_Unchecked(object sender, RoutedEventArgs e) => BaşlangıçtaÇalıştır(false);
    }
}
